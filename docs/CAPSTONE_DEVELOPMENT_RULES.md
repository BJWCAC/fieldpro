...
- **The deal's job number is part of the deal name and always stays.** The four-digit number in a deal name is how the shop and the customer both refer to the visit, so `customerCopyLooksLikeJobNumber()` keeps every shape it is written in — `4641`, `#4641`, `4641-2`, `CAC-4641`, `IA-4641`, `P4641`, and `Rogers WWTP 4641` — and `customerCopyJobRefTokens()` carries the same number through the report body and photo notes, not just the header. A code in the deal name that happens to share that shape (`DR-4500`) is withheld only when the report itself withheld the same code, so the decision rests on evidence rather than shape. `customerCopyKeepTokens()` then narrows the body's keep list to what actually survives in the deal name, so the header and the body can never disagree about the same number: a deal named after a model does not license that model anywhere else.
