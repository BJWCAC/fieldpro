...
- **The copy must never indicate that anything was left out.** No placeholder, no bracketed note, no "not included" line under the copy name in the PDF, no empty label, and no blank section — a customer copy has to read like a report written without those numbers. Each pass marks its removal with `CUSTOMER_COPY_GAP` (a control character that never survives into rendered text) and `closeCustomerCopyGaps()` closes the sentence over it: a labeled identifier goes label and all (a bare `Model:` says as much as a placeholder), values removed side by side leave one hole, brackets that held nothing but a code go with it, an emptied list item takes its separator along, a sentence that lost its opening keeps its capital, a determiner with nothing left to name goes (`Replace the MRC 7000 within 12 months` → `Replace within 12 months`), and a line — or a section heading left standing over nothing — is dropped rather than printed empty. Imperfect grammar is acceptable here; a marker is not.
- **A label is not required.** Reports say "Replaced the Honeywell DR4500A chart recorder" as often as they say "Model: DR4500A", so `redactCustomerCopyBareCodes()` also withholds a code by shape: mixed letters and digits at least four characters long (`DR4500A`, `MRC7000`, `uR1800`, `3051S`, `FMU90-R11CA111AA3A`), a grouped numeric code (`51404671-501`, `24001660-001`), and a spaced model prefix plus code (`MRC 7000`) via `redactCustomerCopySpacedCodes()`. An unlabeled code is cut out of its sentence, which then closes up: `Replaced the Honeywell DR4500A chart recorder in panel 3.` renders as `Replaced the Honeywell chart recorder in panel 3.`
...
- **Show the technician what is left out, and only the technician.** The Report tab renders exactly what the active copy will contain and counts the removed items, so a customer copy is verifiable on screen before it is sent. That count lives in the app around the report, never in the report, the PDF, or the shared text.
- **Every change to the filter runs `node tests/customer-copy-redaction.js`.** The checks lift the redaction block straight out of `src/app.js` and cover all three directions — codes that must go, readings/tags/job references that must stay, and rendered text that must carry no sign of a removal (`renders()` pins the exact wording, `noMarker()` rejects a marker, a leftover label, or the gap character).
