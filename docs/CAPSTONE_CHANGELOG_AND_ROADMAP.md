...
|---------|-----|--------------|
| v376 | #267 | **IA Number spans most of the top row beside GPS; Name full-width below** — Number and GPS share one top row (GPS aligned with the number field); Number uses the remaining width (`1fr` vs ~32% GPS). IA Name sits on its own full-width row under that. |
