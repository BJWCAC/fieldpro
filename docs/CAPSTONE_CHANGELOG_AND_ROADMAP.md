...
|---------|-----|--------------|
| v385 | — | **The technician's photo descriptions now reach the report body** — reported from the field as "I added the photos again, pressed generate. It created the AI descriptions but not what I added for descriptions." `generate()` sent the images, voice notes, video transcript, and the nine report sections to the model but never the text typed on each photo, so a technician who documented the visit in the photo descriptions got a report written from the AI's own observations only. Nothing was lost — the descriptions were still stored and still printed beside each photo in the PDF and on the Report tab — they simply were not an input to the report. The prompt now carries **TECHNICIAN NOTES ON THE EQUIPMENT PHOTOGRAPHED** for every photo that has a description, independent of the four-image budget, and instruction 2 changed from "do not mention photos" to "use every fact from the photo notes in whichever sections they belong to, but do not describe the photos themselves or refer to photo numbers", so the report reads as a report rather than a photo log. Customer copies are unaffected: the report body is still filtered at render time. |
| v385 | — | **AI Observation and AI Synthesis stop disappearing from History** — `addAiPhotoNotes()` writes onto the *report* photo objects (`A.reportPhotos`), while Capture keeps its own objects for the same photos. The background History autosave five seconds after a generate describes `A.photos`, so it wrote the same photo back with `aiDesc: ""` and `synthesis: ""`: the record kept the typed description but silently lost both AI blocks, and every PDF, Continue, or copy rendered from History afterwards was missing them. `generate()` now hands the AI text back to the matching Capture photos by id, the way `updateReportPhotos()` already did, and `mergeHistoryRecord()` gained a per-photo merge (`mergeHistoryPhotoData()`) so a silent autosave keeps each photo's stored `desc`, `label`, `aiDesc`, and `synthesis` when the incoming copy has none — the same "add or update, never blank" rule the record-level keys already followed. Verified in Chrome with stubbed AI calls: before the fix the stored `aiDesc`/`synthesis` were empty seven seconds after generating; after it they survive. |
...
| v385 | Photo descriptions reach the report body; neutral buttons match their card |
