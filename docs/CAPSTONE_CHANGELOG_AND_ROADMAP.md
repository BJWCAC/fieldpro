...
Current live version: v385
Test URL: https://BJWCAC.github.io/fieldpro/FieldPro.html?v=385
```
...
|---------|-----|--------------|
| v385 | — | **A customer copy withholds a part number even when nobody labeled it** — reported from the field as "I just created another customer copy for [deal] 4641 and the report still has the part number for the chart recorder listed". Two holes, both in `redactCustomerCopyText()`. First, a labeled value only matched when its **first** token carried a digit, so `Model: Honeywell DR4500A`, `Part number: Honeywell 51404671-501`, and `Model: Partlow MRC 7000` matched nothing at all and printed in full — the label was there, the brand name in front of the code defeated the pattern. `customerCopyIdValueEnd()` now scans the value instead of matching it: up to two leading brand/series words plus the codes after them, stopping at the first ordinary word or at punctuation, so `Model: DR4500A installed in panel 3` keeps the rest of the sentence and a labeled value with no code in it ("parts used were on hand") is still left alone. Second, **a label was required at all**, and reports say "Replaced the Honeywell DR4500A chart recorder" as often as they say `Model: DR4500A`. `redactCustomerCopyBareCodes()` now withholds a code by shape — mixed letters and digits four characters or longer (`DR4500A`, `MRC7000`, `uR1800`, `3051S`, `FMU90-R11CA111AA3A`), grouped numeric codes (`51404671-501`, `24001660-001`), and a spaced model prefix plus code (`MRC 7000`) — using `[part number not shown on customer copy]` so the sentence around it still reads. Readings are protected by construction rather than by luck: `customerCopyIsUnitReading()` keeps anything with a unit (`24VDC`, `4-20mA`, `0-150inH2O`, `3/4in`, `120VAC/24VDC`), and dates, times, revisions, chemistry (`H2SO4`, `NaOCl`), material grades (`SS316`, `316L`), ratings and standards (`IP65`, `SIL2`, `NEMA Type 4X`, `ISO 17025`), lab values (`MLSS 3200 mg/L`, `TDS 450`), ZIP codes (`Rogers, MN 55374`), and job references (`WO 44821`, `PO 7781`) all stay. Plant tags stay too — an ISA loop tag (`FIT-101`, `AIT-2301`, `FIT101`) or a panel/area tag (`LCP-3`, `MCC-2`, `VFD-1`) tells the customer which instrument was serviced — and the tag exception requires a real ISA function code, which is what keeps a part number like `GK-4471` or `DR-4500` from hiding behind the same shape. Also filtered now: the **deal name** in the PDF header, the on-screen Report header, and the share text, because a deal is usually named after the equipment (`4641 DR4500A recorder calibration`); it is marked `[withheld]` there to fit the header cell, and counted in the withheld total. A pricing line finally keeps its non-prices ("within 12 months", "4 rolls"). One more leak was found in the browser rather than on paper: the word **in** after a spaced code ("Partlow MRC 7000 in panel LCP-3", in the AI Synthesis) was being read as inches, so `customerCopyFollowingWordIsUnit()` no longer counts units that are also ordinary words. The checks moved out of scratch scripts into `tests/customer-copy-redaction.js` (126 checks, both directions, including a full AI-style report and generated sweeps of 1,080 sentences), which every change to the filter must run. |
| v384 | — | **Change a report's photos without rewriting its text** — asked from the field as "how do I recreate the gallery for 4651 without having to create all the text again". `generate()` was the only thing that ever refreshed `A.reportPhotos`, and it always rewrote `A.report` with a fresh AI call, so re-adding a photo to a finished report meant accepting different wording for a report that had already been reviewed and, in most cases, issued. A capture reopened from History does write its edited photo list back to the record on autosave, but `A.reportPhotos` stayed stale in the live session, so the Report tab, the PDF, the Deal attachment, and the WorkDrive upload all kept using the old set until the record was reopened. New **Update Photos on This Report** on Capture (shown once the capture has a finished report) makes the report's photos match Capture and leaves `A.report` byte for byte: it confirms the actual delta first ("Photos: 3 → 4 (1 added)", or "3 → 3 (1 added, 1 removed)" for a swap, rather than a bare count that makes a swap look like a no-op), keeps the Observation and Synthesis already written for photos that are unchanged, and generates AI notes only for photos that have none. The photo-note AI is now a shared `addAiPhotoNotes()` helper — Generate recaptions everything for a fresh report, Update Photos passes `onlyMissing`. Changing the photos sets `A.dealPdfStale` and clears `A.workdrivePdfUrl`, so the next save replaces the copy attached to the Deal instead of leaving the old PDF; uploads run the same way they do after Generate. The Report tab explains the route under Regenerate, and the Regenerate button finally carries the `regen-btn` id the code has always looked for (it was never actually being disabled during a save). Verified in the browser against deal 4651: adding a fourth photo left the stored report text `===` identical at 451 characters while `photoData` went from 3 entries to 4. |
| v383 | — | **Reopening a saved report no longer finds its photos gone** — reported from the field as "opened deal 4641 and no pictures were present". Every History write ran `persistHistoryRecords()`, which called `isStoragePressure()` *before* attempting the save and, if true, blanked `display` on every photo of every other report and cut History to the newest 12 records. `isStoragePressure()` measures **all** of localStorage via `getStorageSize()`, and the cached deal list alone is several MB on a real device, so the threshold was permanently tripped and the destructive trim ran on every autosave even though localStorage had never refused a write. Photo bytes that IndexedDB happened to hold could still be rehydrated; bytes that only existed inline — any device where IndexedDB is unavailable, or a photo whose IndexedDB write had failed — were destroyed, leaving records that still said "3 photos" with nothing to show. Four fixes: (1) `persistHistoryRecords()` writes the record set untouched first and only escalates (drop offloaded bytes → drop remaining bytes → drop oldest records) once `localStorage.setItem` actually throws, so nothing is given up until it has to be; (2) `stripHistoryPhotoDisplay()` keeps inline bytes unless IndexedDB is confirmed to hold that photo id, so freeing space can never destroy the only copy, and the report being written always keeps its own photos; (3) `initPhotoStore()` now reads every key already in the photo store (`fpIdbLoadPersistedIds()`), so "is this photo safe to drop?" is accurate from the first render of a session instead of only for photos that session wrote, and `fpIdbDeletePhotos()` untracks deleted ids; (4) a History save now schedules `migrateHistoryPhotosToIdb()`, moving inline bytes into IndexedDB in the background so localStorage stays small without trimming. Missing photos are also no longer silent: History cards show an **N Photos Not On This Device** chip, reopening a report toasts the count, the Report tab explains what is missing and what survived (descriptions, AI notes, anything already in WorkDrive), Capture cards show an "Image not on this device" placeholder instead of a broken image, and `buildPDF()` skips byte-less photos rather than printing empty framed boxes. Dropping whole reports is now a last resort and says so. Verified against v382 in a stubbed browser: with a 4.66 MB deal cache, 15 visits × 3 photos and no IndexedDB, one routine autosave took v382 from 15 reports/45 photos to 12 reports/3 photos (an older report still claiming "3 photos" with 0 openable); v383 keeps 15/45. |
| v382 | #274 | **Regenerate works on the report's saved capture** — opening a report from History with **View** loads the report and photos but not the Capture fields, so the Report tab's Regenerate rebuilt the report from an empty capture (and a deliberate save then wrote blank sections/notes over the record). Regenerate now reloads that report's saved capture first — photos, voice notes, sections, transcript, deal, location, copy name — unless there is live capture work on screen, so unsaved edits are never overwritten. `continueHist()`, `regenerateReport()`, and `retryQueuedReportGenerate()` now share one `loadHistoryRecordIntoCapture()` helper instead of three near-copies; as a side effect **Continue** loads a record wholesale, so section text from a previous project no longer lingers when the opened record has none. The control also became a full-width `.bg-lg` **Regenerate Report** button — as a small `.bg .bsm` bar it read as a collapsible section header rather than an action. |
| v382 | #272 | **History keeps the full capture when a copy is rendered from it (regression fix)** — tapping a copy-name button on the Report tab after opening a report from History with **View** scheduled a background capture-history save, and `buildCaptureHistoryMeta()` describes the *Capture* tab: with `A.photos` empty it wrote `photos: 0`, `photoData: []`, blank `sections`, and blank `voiceNotes` over the record. Caught by exporting History after generating a customer copy. Two fixes: the meta now falls back to `A.reportPhotos` (same source order the PDF and `generate()` use), and a silent autosave merges through `mergeHistoryRecord()`, which never replaces existing `report`/`voiceNotes`/`sections`/`photoData`/`videos`/`videoTranscript` with empty values — a deliberate save still writes exactly what is on screen. The copy picker also writes only `copyType`/`copyLabel` via `updateCurrentHistory()` instead of rebuilding the record. Verified end to end: View → Customer Copy → Download PDF → toggle copies → wait out the autosaves, then Export History; the record still held the photo with all three text blocks, both sections, voice notes, the transcript, and every part/model/order/serial number and price, while the downloaded customer PDF withheld them all. 17 merge checks added. |
| v382 | #272 | **Model and order numbers count as part numbers on customer copies** — a model number is a part number for this purpose, and Endress+Hauser prints the same identifier as an **order number / order code**, so `Model`/`Mdl`, `Order`/`Ordering` + `number`/`code`, and `Catalog` labels now redact alongside `Serial`, `S/N`, `Part`, and `P/N`. Short values are caught too (`Model 90`), and E+H style codes (`FMU90-R11CA111AA3A`) redact whole. **Job references stay**: `Work order 44821`, `Purchase order 7781`, and prose like "in order to test 2 meters" are untouched, as are unlabeled brand/series text (`Pulsar Ultra 4`), readings, units, dates, and tolerances. Generation prompts now ask for model and order numbers to be labeled too. Redaction checks grew to 40, covering both directions. |
| v381 | #270 | **Customer copies withhold part numbers, serial numbers, and pricing** — a Customer Copy must never carry equipment part numbers, serial numbers, or pricing in any field: report body, the technician's photo description, AI Observation, or AI Synthesis. Internal Copy and Other keep the full detail. Because one report is generated per visit and any copy can be rendered from it later, the filtering happens at **render** time rather than by weakening what was captured: `buildPDF()` filters whenever `isCustomerCopyLabel()` matches (covering local download, WorkDrive, Deal attachment, and History export in one place), `buildReportExportText()` filters share/email/clipboard, and the Report tab shows exactly what the active copy will contain plus a count of withheld items. `redactCustomerCopyText()` keeps the label and replaces the value (`Serial: [not shown on customer copy]`, `[pricing not shown on customer copy]`) so nothing reads doctored; calibration readings, engineering units, percentages, dates, and durations are deliberately untouched. Generation prompts now require part/serial numbers to be written with a label, which is what makes them detectable in later copies. A typed `Other` name containing "customer" is treated as a customer copy. The Zoho Deal note stays unfiltered — internal CRM record, never handed to a customer. Verified end to end: from one stored report, the customer PDF contained none of `4X-9921B`, `220-4471`, `GK-4471`, `ABC1234`, `$412.00`, `$18,750`, `950` (and no `$` at all) while keeping `1200 GPM`, `4-20 mA`, `0-150 in H2O`, `0.5%`, `pH 7.01`; the internal PDF kept every one of them. |
| v380 | #270 | **Report copy names + no deal amount in the report header** — generating a report more than once produced indistinguishable copies of the original: every copy shared one PDF/WorkDrive/Deal-attachment name, and `A.dealPdfAttached` short-circuited the Deal attachment, so the Deal kept the *first* PDF no matter how often the report was regenerated. Capture (above Generate AI Report) and Report (above PDF Options) now share a **Report Copy Name** picker — **Customer Copy** / **Internal Copy** presets plus **Other** for a typed name, last choice remembered per device in `fp_report_copy`. The label appears in the PDF header bar and feeds the local PDF filename, `workdrivePdfFileName()` (WorkDrive + Deal attachment), the Zoho note title, the note/share/email text, the on-screen Report header, both checklists, and a History chip. Regenerating the **same** copy name replaces that copy (WorkDrive overrides by name; the Deal PDF is deleted then re-attached via the new `delete_deal_attachment` proxy action, proxy build **290**); a **different** copy name is filed alongside it. `Other` is a required field — Generate and Download PDF are blocked with a toast and light-red highlight until it is named. Separately, the **deal amount is gone** from the PDF deal header block (that cell now shows `DATE`), the on-screen Report header, the Zoho note header, and the AI generation prompt, so pricing cannot reach a customer copy. Pre-v380 History records have no copy name and keep their old unlabeled filenames. |
...
| v383 | History keeps its photos — trimming only after a real quota failure |
| v384 | Update a report's photos without rewriting its text |
| v385 | Customer copies withhold part numbers even when unlabeled |
