...
**Keep these in sync.** If you improve the rules here, update `MODEL_AI_SPECS_SYSTEM_PROMPT` in `src/app.js` to match, and vice versa. This file is the source of truth; the in-app prompt is a condensed version of it tuned to fit an API call.
