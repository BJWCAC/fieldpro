...
Rules:
- **Search the web first — deeply.** Before writing, do a *thorough, multi-source* lookup rather than a single query answered from memory. Run several targeted searches: `"<brand> <model> datasheet"`, `"<brand> <model> accuracy specification"`, the operating/instruction manual, and (when the accuracy basis is unclear) a query for `% of reading` vs `% of span`. Decode the **full** model/order code — don't stop at the family name. Prefer the **manufacturer's own datasheet**, and when the first source is ambiguous, incomplete, or looks wrong, open a **second independent source and cross-check** before committing to a figure. Base every number on what you actually found. When `Asset_Brand` is a generic "Other" value but `If_Asset_Brand_Other_explain` names the actual brand/manufacturer, use that explain text as the brand for the search and the spec. Cite the source in the `[AI-gen]` line (manufacturer or domain). Only fall back to `NOT VERIFIED` after a genuine search does not surface a trustworthy figure. CapStone's auto-generation enables web-search grounding on both the Gemini (`google_search`) and Claude (`web_search`) draft calls — keep `search:true` on those calls in `fetchModelAiSpecsDraft()`.
...
**Model tier & token budget (why the field can look thinner than a direct Gemini search):** the Gemini web app answers a brand+model question with a Pro-tier model, deep "thinking", and multi-step browsing, and returns a full multi-paragraph write-up. CapStone must fit a 2000-char calibration summary, so the output is intentionally terser — but the *research* should be just as good. To keep it so, the Gemini draft call in `src/app.js` sets `preferQuality:true` (resolves a Pro model such as `gemini-2.5-pro` via `GEMINI_QUALITY_MODEL_PREFERENCE`, falling back to the Flash list only when the key lacks Pro access or quota) and uses a generous `maxTok` (~4096) plus a longer per-attempt timeout (~90 s). The generous token budget matters because Gemini 2.5 "thinking" tokens count against `maxOutputTokens` and a deep multi-source search spends a lot of them; a tight budget gets eaten by reasoning/browsing and yields a truncated/empty spec, so the budget sits well above the ~1900-char (~500-token) output. The longer timeout gives the deep search room to finish before the AbortController fires. Keep `search:true` on the draft calls so the model reads live datasheets rather than working from memory, and keep the prompt's multi-query / cross-check instructions in step with the "Search the web first — deeply" bullet in §2.
