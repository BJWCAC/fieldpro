...
var ACCOUNTS_MAP_PROXY="/.netlify/functions/zoho-proxy";
...
var FP_VERSION="211";
A.accountsMap={accounts:[],meetings:[],meetingLoadError:"",locations:[],missing:[],loading:false,map:null,markers:[],infoWindow:null,googleReady:null,pipeline:"All",stage:"",status:"Active",meetingStatus:"All",search:"",lastLoaded:0};
...
function go(n){
  ["deals","accounts-map","capture","assets","inbox","report","history","settings"].forEach(function(x){
    var p=el("p-"+x),t=el("t-"+x);
    if(p)p.classList.toggle("on",x===n);
    if(t)t.classList.toggle("on",x===n);
  });
  if(n==="accounts-map"&&typeof ensureAccountsMapStarted==="function")ensureAccountsMapStarted();
...
function loadDealsFromCache(){
  ...
  p=p.map(function(d){return{id:d.id,Deal_Name:strVal(d.Deal_Name),Account_Name:strVal(d.Account_Name),Account_Id:d.Account_Id||"",Stage:strVal(d.Stage),Pipeline:strVal(d.Pipeline),Amount:d.Amount||null,Description:strVal(d.Description),Owner:strVal(d.Owner),Closing_Date:strVal(d.Closing_Date)};});
  A.deals=p;
  if(typeof renderDeals==="function")renderDeals();
...
window.applyAccountsMapFilters=applyAccountsMapFilters;
window.clearAccountsGeoCache=clearAccountsGeoCache;
...
function importCSV(input){
  ...
  reader.onload=function(e){
    ...
    for(var i=1;i<lines.length;i++){var vals=parseCSVLine(lines[i]);if(!vals.length)continue;var row={};headers.forEach(function(h,idx){row[h.trim()]=(vals[idx]||"").trim();});var deal={id:row["Deal Id"]||("csv-"+i),Deal_Name:row["Deal Name"]||"",Account_Name:row["Account Name"]||"",Account_Id:row["Account Id"]||"",Stage:row["Stage"]||"",Pipeline:row["Pipeline"]||"",Amount:parseFloat(row["Amount"])||null,Description:row["Description"]||"",Owner:row["Deal Owner"]||"",Closing_Date:row["Closing Date"]||null};if(deal.Account_Name||deal.Deal_Name)deals.push(deal);}
...
// ACCOUNTS MAP
var ACCOUNTS_MAP_CACHE_KEY="capstone_geo_cache";
var ACCOUNTS_MAP_CENTER={lat:44.5,lng:-93.5};
var ACCOUNTS_MAP_ZOOM=7;
var ACCOUNTS_MAP_STAGE_PALETTE=["#10b981","#00c0a0","#1a8a7a","#3b82f6","#8b5cf6","#f59e0b","#ef4444","#64748b","#14b8a6","#22c55e"];
var ACCOUNTS_MAP_STAGE_COLORS={"qualification":"#3b82f6","needs analysis":"#8b5cf6","value proposition":"#00c0a0","identify decision makers":"#14b8a6","proposal/price quote":"#f59e0b","negotiation/review":"#f97316","closed won":"#10b981","closed lost":"#ef4444"};
function accountsMapState(){if(!A.accountsMap)A.accountsMap={accounts:[],meetings:[],meetingLoadError:"",locations:[],missing:[],loading:false,map:null,markers:[],infoWindow:null,googleReady:null,pipeline:"All",stage:"",status:"Active",meetingStatus:"All",search:"",lastLoaded:0};return A.accountsMap;}
function accountsMapText(v){if(v===null||typeof v==="undefined")return"";if(typeof v==="object")return String(v.name||v.display_value||v.value||v.id||"").trim();return String(v).trim();}
function accountsMapProxyUrl(){return ACCOUNTS_MAP_PROXY||PROXY;}
function setAccountsMapStatus(msg,isErr){var e=el("accounts-map-load-status");if(!e)return;e.innerHTML=esc(msg||"");e.style.color=isErr?"var(--red)":"var(--dim)";}
function accountsMapHasGoogleKey(){return !!String(GOOGLE_MAPS_API_KEY||"").trim();}
async function refreshAccountsMapZohoToken(){var r=await fetchWithTimeout(accountsMapProxyUrl(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"refresh_token",refresh_token:ZOHO_REFRESH,client_id:ZOHO_CLIENT,client_secret:ZOHO_SECRET})},30000);var d=await r.json();if(d.access_token){A.zohoToken=d.access_token;return true;}return false;}
function normalizeAccountsMapDeal(rec){function str(v){return accountsMapText(v);}return{id:rec.id,Deal_Name:str(rec.Deal_Name),Account_Name:str(rec.Account_Name),Account_Id:(rec.Account_Name&&rec.Account_Name.id)||"",Stage:str(rec.Stage),Pipeline:str(rec.Pipeline),Amount:rec.Amount||null,Description:str(rec.Description),Owner:str(rec.Owner&&rec.Owner.name?rec.Owner.name:rec.Owner),Closing_Date:str(rec.Closing_Date)};}
async function loadAccountsMapDeals(){var allDeals=[],page=1,hasMore=true;while(hasMore&&page<=10){var r=await fetchWithTimeout(accountsMapProxyUrl(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"get_deals",token:A.zohoToken,page:page})},ZOHO_FETCH_MS);if(!r.ok){if(r.status===401){await refreshAccountsMapZohoToken();r=await fetchWithTimeout(accountsMapProxyUrl(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"get_deals",token:A.zohoToken,page:page})},ZOHO_FETCH_MS);}if(!r.ok)throw new Error("Deal lookup failed: "+r.status);}var d=await r.json();(d.data||[]).forEach(function(rec){allDeals.push(normalizeAccountsMapDeal(rec));});hasMore=d.info&&d.info.more_records;page++;}if(allDeals.length){A.deals=allDeals;try{localStorage.setItem("fp_deals",JSON.stringify(allDeals));}catch(e){}badge("tb-deals",allDeals.length);if(typeof renderDeals==="function")renderDeals();}}
function normalizeAccountsMapAccount(rec){return{id:rec.id||"",Account_Name:accountsMapText(rec.Account_Name),Account_Status:accountsMapText(rec.Account_Status),Phone:accountsMapText(rec.Phone),Latitude_Longitude:accountsMapText(rec.Latitude_Longitude),googlemapreports__Latitude:accountsMapText(rec.googlemapreports__Latitude),googlemapreports__Longitude:accountsMapText(rec.googlemapreports__Longitude),Shipping_Street:accountsMapText(rec.Shipping_Street),Shipping_Street_2:accountsMapText(rec.Shipping_Street_2),Shipping_City:accountsMapText(rec.Shipping_City),Shipping_State:accountsMapText(rec.Shipping_State),Shipping_Code:accountsMapText(rec.Shipping_Code),Billing_Street:accountsMapText(rec.Billing_Street),Billing_City:accountsMapText(rec.Billing_City),Billing_State:accountsMapText(rec.Billing_State),Billing_Code:accountsMapText(rec.Billing_Code)};}
async function fetchAccountsMapRecords(){var all=[],page=1,hasMore=true;while(hasMore&&page<=10){var r=await fetchWithTimeout(accountsMapProxyUrl(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"get_accounts_map",token:A.zohoToken,page:page})},ZOHO_FETCH_MS);if(!r.ok){if(r.status===401){await refreshAccountsMapZohoToken();r=await fetchWithTimeout(accountsMapProxyUrl(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"get_accounts_map",token:A.zohoToken,page:page})},ZOHO_FETCH_MS);}if(!r.ok)throw new Error("Account lookup failed: "+r.status);}var d=await r.json();(d.data||[]).forEach(function(rec){all.push(normalizeAccountsMapAccount(rec));});hasMore=d.info&&d.info.more_records;page++;setAccountsMapStatus("Loading accounts... "+all.length+" records");}return all;}
function normalizeAccountsMapMeeting(rec){function str(v){return accountsMapText(v);}var what=rec.What_Id||{},who=rec.Who_Id||{};return{id:rec.id||"",title:str(rec.Event_Title||rec.Subject||rec.Meeting_Title)||"Meeting",status:str(rec.Meeting_Status||rec.Event_Status||rec.Status)||"No status",start:str(rec.Start_DateTime),end:str(rec.End_DateTime),whatId:what&&what.id||"",whatName:str(what),whatModule:str(what&&what.module),whoId:who&&who.id||"",whoName:str(who),owner:str(rec.Owner&&rec.Owner.name?rec.Owner.name:rec.Owner),description:str(rec.Description)};}
async function fetchAccountsMapMeetings(){var all=[],page=1,hasMore=true;while(hasMore&&page<=10){var r=await fetchWithTimeout(accountsMapProxyUrl(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"get_meetings_map",token:A.zohoToken,page:page})},ZOHO_FETCH_MS);if(!r.ok){if(r.status===401){await refreshAccountsMapZohoToken();r=await fetchWithTimeout(accountsMapProxyUrl(),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"get_meetings_map",token:A.zohoToken,page:page})},ZOHO_FETCH_MS);}if(!r.ok)throw new Error("Meeting lookup failed: "+r.status);}var d=await r.json();(d.data||[]).forEach(function(rec){all.push(normalizeAccountsMapMeeting(rec));});hasMore=d.info&&d.info.more_records;page++;setAccountsMapStatus("Loading meetings... "+all.length+" records");}return all;}
function accountsMapAddress(account,prefix){var fields=prefix==="Shipping"?["Shipping_Street","Shipping_Street_2","Shipping_City","Shipping_State","Shipping_Code"]:["Billing_Street","Billing_City","Billing_State","Billing_Code"];return fields.map(function(f){return accountsMapText(account[f]);}).filter(Boolean).join(", ");}
function parseAccountsMapDirectCoords(account){var lat=parseFloat(account.googlemapreports__Latitude);var lng=parseFloat(account.googlemapreports__Longitude);if(Number.isFinite(lat)&&Number.isFinite(lng)&&lat>=-90&&lat<=90&&lng>=-180&&lng<=180)return{lat:lat,lng:lng};return null;}
function loadAccountsGeoCache(){try{var raw=localStorage.getItem(ACCOUNTS_MAP_CACHE_KEY);return raw?JSON.parse(raw):{};}catch(e){return{};}}
function saveAccountsGeoCache(cache){try{localStorage.setItem(ACCOUNTS_MAP_CACHE_KEY,JSON.stringify(cache));}catch(e){}}
function accountsGeoCacheKey(address){return String(address||"").trim().toLowerCase().replace(/\s+/g," ");}
function geocodeAccountsMapAddress(address){return new Promise(function(resolve){var cache=loadAccountsGeoCache(),key=accountsGeoCacheKey(address),hit=cache[key];if(hit&&Number.isFinite(hit.lat)&&Number.isFinite(hit.lng)){resolve({lat:hit.lat,lng:hit.lng,cached:true});return;}if(!window.google||!google.maps||!google.maps.Geocoder){resolve(null);return;}var geocoder=new google.maps.Geocoder();geocoder.geocode({address:address,region:"US"},function(results,status){if(status==="OK"&&results&&results[0]&&results[0].geometry&&results[0].geometry.location){var loc=results[0].geometry.location;var out={lat:loc.lat(),lng:loc.lng(),cached:false};cache[key]={lat:out.lat,lng:out.lng,ts:Date.now()};saveAccountsGeoCache(cache);resolve(out);}else resolve(null);});});}
async function ensureGoogleMapsLoaded(){if(window.google&&google.maps&&google.maps.Map)return true;var s=accountsMapState();if(s.googleReady)return s.googleReady;if(!accountsMapHasGoogleKey())throw new Error("Google Maps API key missing. Add it to GOOGLE_MAPS_API_KEY in src/app.js.");s.googleReady=new Promise(function(resolve,reject){var cb="__capstoneAccountsMapGoogleReady";window[cb]=function(){resolve(true);};var script=document.createElement("script");script.async=true;script.defer=true;script.onerror=function(){reject(new Error("Google Maps JavaScript API failed to load"));};script.src="https://maps.googleapis.com/maps/api/js?key="+encodeURIComponent(GOOGLE_MAPS_API_KEY)+"&callback="+cb+"&loading=async";document.head.appendChild(script);});return s.googleReady;}
function initAccountsMap(){var s=accountsMapState(),canvas=el("accounts-map-canvas");if(!canvas||s.map||!window.google||!google.maps)return;s.map=new google.maps.Map(canvas,{center:ACCOUNTS_MAP_CENTER,zoom:ACCOUNTS_MAP_ZOOM,mapTypeControl:false,streetViewControl:false,fullscreenControl:true,styles:[{elementType:"geometry",stylers:[{color:"#0d1a18"}]},{elementType:"labels.text.fill",stylers:[{color:"#94a3b8"}]},{elementType:"labels.text.stroke",stylers:[{color:"#071514"}]},{featureType:"road",elementType:"geometry",stylers:[{color:"#1a3030"}]},{featureType:"water",elementType:"geometry",stylers:[{color:"#061f2a"}]}]});s.infoWindow=new google.maps.InfoWindow();}
function relatedDealsForAccount(account){var aid=account.id,an=(account.Account_Name||"").toLowerCase();return(A.deals||[]).filter(function(d){return(aid&&d.Account_Id&&d.Account_Id===aid)||((d.Account_Name||"").toLowerCase()===an&&an);});}
function relatedMeetingsForAccount(account,relatedDeals){var aid=account.id,an=(account.Account_Name||"").toLowerCase(),deals=relatedDeals||relatedDealsForAccount(account),dealIds=deals.map(function(d){return d.id;}).filter(Boolean),dealNames=deals.map(function(d){return(d.Deal_Name||"").toLowerCase();}).filter(Boolean);return(accountsMapState().meetings||[]).filter(function(m){var wid=m.whatId,wname=(m.whatName||"").toLowerCase();if(aid&&wid===aid)return true;if(wid&&dealIds.indexOf(wid)>=0)return true;if(wname&&wname===an)return true;if(wname&&dealNames.indexOf(wname)>=0)return true;return false;});}
function normalizeAccountsMapPipeline(p){var raw=accountsMapText(p),low=raw.toLowerCase().replace(/[\s_-]+/g," ");if(low.indexOf("inhouse")>=0||low.indexOf("in house")>=0||low.indexOf("lab")>=0)return"InHouse Lab";if(low.indexOf("job")>=0)return"Jobs";return raw;}
function stageColor(stage){var key=accountsMapText(stage).toLowerCase();if(ACCOUNTS_MAP_STAGE_COLORS[key])return ACCOUNTS_MAP_STAGE_COLORS[key];if(!key)return"#64748b";var h=0;for(var i=0;i<key.length;i++)h=(h*31+key.charCodeAt(i))%997;return ACCOUNTS_MAP_STAGE_PALETTE[h%ACCOUNTS_MAP_STAGE_PALETTE.length];}
function pipelineColor(pipeline){pipeline=normalizeAccountsMapPipeline(pipeline);if(pipeline==="Jobs")return"#00c0a0";if(pipeline==="InHouse Lab")return"#f59e0b";return"#64748b";}
function primaryDealMeta(relatedDeals){var deals=relatedDeals||[],first=deals[0]||{};return{stage:first.Stage||"",pipeline:normalizeAccountsMapPipeline(first.Pipeline||""),deal:first};}
async function resolveAccountsMapLocation(account){var related=relatedDealsForAccount(account),meetings=relatedMeetingsForAccount(account,related),meta=primaryDealMeta(related),shipping=accountsMapAddress(account,"Shipping"),billing=accountsMapAddress(account,"Billing"),direct=parseAccountsMapDirectCoords(account),base={account:account,relatedDeals:related,relatedMeetings:meetings,dealStage:meta.stage,pipeline:meta.pipeline,siteCoordsRaw:account.Latitude_Longitude||""};if(direct)return Object.assign(base,{lat:direct.lat,lng:direct.lng,source:"coords",displayAddress:shipping||"Coordinates on file",addressLabel:"Stored coordinates"});if(shipping){var shipGeo=await geocodeAccountsMapAddress(shipping);if(shipGeo)return Object.assign(base,{lat:shipGeo.lat,lng:shipGeo.lng,source:"shipping",displayAddress:shipping,addressLabel:"Shipping address",cached:shipGeo.cached});if(billing){var billAfterShip=await geocodeAccountsMapAddress(billing);if(billAfterShip)return Object.assign(base,{lat:billAfterShip.lat,lng:billAfterShip.lng,source:"billing",displayAddress:billing+" (billing address)",addressLabel:"Billing address",cached:billAfterShip.cached});}return{account:account,relatedDeals:related,relatedMeetings:meetings,reason:"Shipping geocode failed"+(billing?"; billing geocode failed":"")};}if(billing){var billGeo=await geocodeAccountsMapAddress(billing);if(billGeo)return Object.assign(base,{lat:billGeo.lat,lng:billGeo.lng,source:"billing",displayAddress:billing+" (billing address)",addressLabel:"Billing address",cached:billGeo.cached});return{account:account,relatedDeals:related,relatedMeetings:meetings,reason:"Billing geocode failed"};}return{account:account,relatedDeals:related,relatedMeetings:meetings,reason:"No stored coordinates, shipping address, or billing address"};}
async function resolveAccountsMapLocations(){var s=accountsMapState(),locations=[],missing=[];for(var i=0;i<s.accounts.length;i++){var resolved=await resolveAccountsMapLocation(s.accounts[i]);if(resolved&&Number.isFinite(resolved.lat)&&Number.isFinite(resolved.lng))locations.push(resolved);else missing.push(resolved||{account:s.accounts[i],relatedDeals:relatedDealsForAccount(s.accounts[i]),reason:"No location"});if((i+1)%10===0||i===s.accounts.length-1)setAccountsMapStatus("Resolving account locations... "+(i+1)+" of "+s.accounts.length);}s.locations=locations;s.missing=missing;}
function populateAccountsMapStageFilter(){var sel=el("accounts-map-stage");if(!sel)return;var current=sel.value;var stages=Array.from(new Set((A.deals||[]).map(function(d){return d.Stage;}).filter(Boolean))).sort();sel.innerHTML="<option value=''>All Stages</option>"+stages.map(function(stage){return"<option value='"+esc(stage)+"'>"+esc(stage)+"</option>";}).join("");sel.value=stages.indexOf(current)>=0?current:"";}
function populateAccountsMapMeetingStatusFilter(){var sel=el("accounts-map-meeting-status");if(!sel)return;var current=sel.value||"All";var statuses=Array.from(new Set((accountsMapState().meetings||[]).map(function(m){return m.status||"No status";}).filter(Boolean))).sort();sel.innerHTML="<option value='All'>All Meetings</option>"+statuses.map(function(status){return"<option value='"+esc(status)+"'>"+esc(status)+"</option>";}).join("");sel.value=statuses.indexOf(current)>=0?current:"All";}
function accountStatusMatches(account,status){if(status==="All")return true;var st=(account.Account_Status||"").toLowerCase();if(status==="Inactive")return st.indexOf("inactive")>=0;if(!st)return true;return st.indexOf("inactive")<0&&st.indexOf("active")>=0;}
function accountMatchesMapFilters(account,relatedDeals,relatedMeetings){var s=accountsMapState(),q=(s.search||"").toLowerCase();if(q&&(account.Account_Name||"").toLowerCase().indexOf(q)<0)return false;if(!accountStatusMatches(account,s.status||"Active"))return false;var deals=relatedDeals||[],meetings=relatedMeetings||relatedMeetingsForAccount(account,deals);if(s.stage&&deals.length&&!deals.some(function(d){return(d.Stage||"")===s.stage;}))return false;if(s.stage&&!deals.length)return false;if(s.pipeline&&s.pipeline!=="All"){if(!deals.length)return false;if(!deals.some(function(d){return normalizeAccountsMapPipeline(d.Pipeline)===s.pipeline;}))return false;}if(s.meetingStatus&&s.meetingStatus!=="All"){if(!meetings.length)return false;if(!meetings.some(function(m){return(m.status||"No status")===s.meetingStatus;}))return false;}return true;}
function filteredAccountsMapLocations(){var s=accountsMapState();return s.locations.filter(function(loc){return accountMatchesMapFilters(loc.account,loc.relatedDeals,loc.relatedMeetings);});}
function filteredAccountsMapMissing(){var s=accountsMapState();return s.missing.filter(function(item){return accountMatchesMapFilters(item.account,item.relatedDeals,item.relatedMeetings);});}
function accountsMapMarkerIcon(loc){var fill=stageColor(loc.dealStage),stroke=pipelineColor(loc.pipeline);var svg="<svg xmlns='http://www.w3.org/2000/svg' width='30' height='42' viewBox='0 0 30 42'><path d='M15 1C7.3 1 1 7.3 1 15c0 10.5 14 26 14 26s14-15.5 14-26C29 7.3 22.7 1 15 1z' fill='"+fill+"' stroke='"+stroke+"' stroke-width='3'/><circle cx='15' cy='15' r='5' fill='#ffffff' fill-opacity='.95'/></svg>";return{url:"data:image/svg+xml;charset=UTF-8,"+encodeURIComponent(svg),scaledSize:new google.maps.Size(30,42),anchor:new google.maps.Point(15,42)};}
function accountsMapPopupHtml(loc){var a=loc.account,stage=loc.dealStage||"No stage",pipeline=loc.pipeline||"No pipeline";var deals=(loc.relatedDeals||[]).slice(0,3).map(function(d){return esc(d.Deal_Name||d.Stage||"Deal");}).join("<br>");var meetings=(loc.relatedMeetings||[]).slice().sort(function(a,b){return String(a.start||"").localeCompare(String(b.start||""));}).slice(0,4).map(function(m){return"<div class='accounts-map-meeting-row'><strong>"+esc(m.title||"Meeting")+"</strong><br><span>"+esc(m.status||"No status")+(m.start?" - "+esc(new Date(m.start).toLocaleString()):"")+"</span></div>";}).join("");var h="<div class='accounts-map-popup'><div class='accounts-map-popup-title'>"+esc(a.Account_Name||"Account")+"</div>";if(a.Account_Status)h+="<div>"+esc(a.Account_Status)+"</div>";if(a.Phone)h+="<div>"+esc(a.Phone)+"</div>";h+="<div class='accounts-map-popup-row'><div class='accounts-map-popup-label'>Placed by</div>"+esc(loc.addressLabel||loc.source||"Location")+"</div>";h+="<div class='accounts-map-popup-row'><div class='accounts-map-popup-label'>Address</div>"+esc(loc.displayAddress||"Coordinates on file")+"</div>";if(loc.siteCoordsRaw)h+="<div class='accounts-map-popup-row'><div class='accounts-map-popup-label'>Site coords:</div>"+esc(loc.siteCoordsRaw)+"</div>";h+="<div class='accounts-map-popup-row'><span class='accounts-map-stage-chip' style='background:"+stageColor(stage)+"'>"+esc(stage)+"</span></div>";h+="<div class='accounts-map-popup-row'><div class='accounts-map-popup-label'>Pipeline</div>"+esc(pipeline)+"</div>";if(deals)h+="<div class='accounts-map-popup-row'><div class='accounts-map-popup-label'>Deals</div>"+deals+"</div>";if(meetings)h+="<div class='accounts-map-popup-row'><div class='accounts-map-popup-label'>Meetings</div>"+meetings+"</div>";h+="</div>";return h;}
function renderAccountsMapMarkers(){var s=accountsMapState();if(!s.map)return;s.markers.forEach(function(m){m.setMap(null);});s.markers=[];var visible=filteredAccountsMapLocations();var bounds=window.google&&google.maps?new google.maps.LatLngBounds():null;visible.forEach(function(loc){var marker=new google.maps.Marker({position:{lat:loc.lat,lng:loc.lng},map:s.map,title:loc.account.Account_Name||"Account",icon:accountsMapMarkerIcon(loc)});marker.addListener("click",function(){s.infoWindow.setContent(accountsMapPopupHtml(loc));s.infoWindow.open(s.map,marker);});s.markers.push(marker);if(bounds)bounds.extend(marker.getPosition());});if(visible.length>1&&bounds)s.map.fitBounds(bounds,{top:28,right:28,bottom:28,left:28});else if(visible.length===1)s.map.setCenter({lat:visible[0].lat,lng:visible[0].lng});else{s.map.setCenter(ACCOUNTS_MAP_CENTER);s.map.setZoom(ACCOUNTS_MAP_ZOOM);}}
function renderAccountsMapLegend(){var box=el("accounts-map-legend");if(!box)return;var visible=filteredAccountsMapLocations();var stages=Array.from(new Set(visible.map(function(loc){return loc.dealStage||"No stage";}))).slice(0,12);var html="<span class='accounts-map-legend-chip'><span class='accounts-map-dot' style='background:#00c0a0;border-color:#00c0a0'></span>Jobs pipeline</span><span class='accounts-map-legend-chip'><span class='accounts-map-dot' style='background:#f59e0b;border-color:#f59e0b'></span>InHouse Lab pipeline</span>";stages.forEach(function(stage){html+="<span class='accounts-map-legend-chip'><span class='accounts-map-dot' style='background:"+stageColor(stage)+"'></span>"+esc(stage)+"</span>";});box.innerHTML=html;}
function renderAccountsMapMissing(){var list=el("accounts-map-missing-list"),count=el("accounts-map-missing-count");var missing=filteredAccountsMapMissing();if(count)count.textContent=missing.length;if(!list)return;if(!missing.length){list.innerHTML="<div style='font-size:12px;color:var(--dim)'>No missing accounts for the current filters.</div>";return;}list.innerHTML=missing.slice(0,80).map(function(item){var a=item.account||{};var status=a.Account_Status?"<div>Status: "+esc(a.Account_Status)+"</div>":"";var meetingCount=item.relatedMeetings&&item.relatedMeetings.length?"<div>Meetings: "+item.relatedMeetings.length+"</div>":"";return"<div class='accounts-map-missing-item'><div class='accounts-map-missing-name'>"+esc(a.Account_Name||"Account")+"</div>"+status+meetingCount+"<div>"+esc(item.reason||"No location")+"</div></div>";}).join("")+(missing.length>80?"<div class='accounts-map-missing-item'>"+(missing.length-80)+" more hidden by panel limit.</div>":"");}
function renderAccountsMapStatusBar(){var bar=el("accounts-map-status-bar");if(!bar)return;var s=accountsMapState(),visible=filteredAccountsMapLocations(),missing=filteredAccountsMapMissing();var filters=["Pipeline: "+(s.pipeline||"All"),"Status: "+(s.status||"Active"),"Stage: "+(s.stage||"All"),"Meeting Status: "+(s.meetingStatus||"All")];if(s.search)filters.push("Search: "+s.search);bar.textContent=visible.length+" pins shown, "+missing.length+" missing ("+s.locations.length+" placed / "+s.accounts.length+" accounts loaded, "+(s.meetings||[]).length+" meetings). "+filters.join(" | ");}
function applyAccountsMapFilters(){var s=accountsMapState();s.search=(el("accounts-map-search")||{value:""}).value.trim();s.pipeline=(el("accounts-map-pipeline")||{value:"All"}).value||"All";s.status=(el("accounts-map-status")||{value:"Active"}).value||"Active";s.stage=(el("accounts-map-stage")||{value:""}).value||"";s.meetingStatus=(el("accounts-map-meeting-status")||{value:"All"}).value||"All";renderAccountsMapMarkers();renderAccountsMapLegend();renderAccountsMapMissing();renderAccountsMapStatusBar();}
async function loadAccountsMap(){var s=accountsMapState(),btn=el("accounts-map-refresh-btn");if(s.loading)return;s.loading=true;if(btn){btn.disabled=true;btn.textContent="Loading...";}try{if(!accountsMapHasGoogleKey())throw new Error("Google Maps API key missing. Add it to GOOGLE_MAPS_API_KEY in src/app.js before using the map.");setAccountsMapStatus("Loading Google Maps...");await ensureGoogleMapsLoaded();initAccountsMap();setAccountsMapStatus("Refreshing Zoho token...");var tokOk=await refreshAccountsMapZohoToken();if(!tokOk)throw new Error("Zoho token refresh failed");setAccountsMapStatus("Loading deal stages and pipelines...");await loadAccountsMapDeals();setAccountsMapStatus("Loading accounts...");s.accounts=await fetchAccountsMapRecords();try{setAccountsMapStatus("Loading meetings...");s.meetings=await fetchAccountsMapMeetings();s.meetingLoadError="";localStorage.setItem("fp_accounts_map_meetings",JSON.stringify(s.meetings));}catch(meetingErr){s.meetings=[];s.meetingLoadError=meetingErr.message||String(meetingErr);}try{localStorage.setItem("fp_accounts_map",JSON.stringify(s.accounts));}catch(e){}await resolveAccountsMapLocations();s.lastLoaded=Date.now();populateAccountsMapStageFilter();populateAccountsMapMeetingStatusFilter();applyAccountsMapFilters();setAccountsMapStatus("Loaded "+s.accounts.length+" accounts, "+(s.meetings||[]).length+" meetings - "+s.locations.length+" placed, "+s.missing.length+" missing."+(s.meetingLoadError?" Meetings unavailable: "+s.meetingLoadError:""));}catch(e){setAccountsMapStatus(e.message||String(e),true);}finally{s.loading=false;if(btn){btn.disabled=false;btn.textContent="Refresh Accounts";}}}
function ensureAccountsMapStarted(){var s=accountsMapState();if(s.lastLoaded||s.loading){setTimeout(function(){if(s.map&&window.google)google.maps.event.trigger(s.map,"resize");},50);return;}try{var cached=localStorage.getItem("fp_accounts_map");if(cached){s.accounts=JSON.parse(cached)||[];try{s.meetings=JSON.parse(localStorage.getItem("fp_accounts_map_meetings")||"[]")||[];}catch(e2){s.meetings=[];}if(s.accounts.length&&accountsMapHasGoogleKey()){ensureGoogleMapsLoaded().then(function(){initAccountsMap();return resolveAccountsMapLocations();}).then(function(){populateAccountsMapStageFilter();populateAccountsMapMeetingStatusFilter();applyAccountsMapFilters();setAccountsMapStatus("Showing "+s.accounts.length+" cached accounts and "+(s.meetings||[]).length+" cached meetings. Tap Refresh Accounts for current Zoho data.");}).catch(function(e){setAccountsMapStatus(e.message,true);});s.lastLoaded=Date.now();return;}}}catch(e){}loadAccountsMap();}
function clearAccountsGeoCache(){try{localStorage.removeItem(ACCOUNTS_MAP_CACHE_KEY);}catch(e){}showToast("Accounts map geocode cache cleared",2500);var s=accountsMapState();if(s.accounts&&s.accounts.length){s.lastLoaded=0;loadAccountsMap();}}
...
function setAssetInput(id,val){var e=el(id);if(e){e.value=val||"";if(id.indexOf("asset-")===0&&id!=="asset-status"&&typeof updateAssetSaveState==="function"){setTimeout(updateAssetSaveState,0);if(id!=="asset-draft-status")scheduleAssetDraftSave();}}}
...
function buildAssetDraft(){var fields={};assetFieldIdsToClear().concat(["asset-search"]).forEach(function(id){fields[id]=assetInput(id);});return{version:1,savedAt:new Date().toISOString(),deal:A.sel||null,location:A.location||null,mode:A.asset.mode,currentAssetId:A.asset.currentAssetId,activeDealKey:A.asset.activeDealKey,loadedOriginal:A.asset.loadedOriginal,replacementMode:A.asset.replacementMode,photos:A.asset.photos,lastUploadedPhotoFingerprints:A.asset.lastUploadedPhotoFingerprints,fields:fields};}
...
function restoreAssetDraft(d){if(!d)return;A.sel=d.deal||A.sel;A.location=d.location||A.location;A.asset.mode=d.mode||"add";A.asset.currentAssetId=d.currentAssetId||null;A.asset.activeDealKey=d.activeDealKey||selectedAssetDealKey();A.asset.loadedOriginal=d.loadedOriginal||null;A.asset.replacementMode=!!d.replacementMode;A.asset.photos=d.photos||[];A.asset.lastUploadedPhotoFingerprints=d.lastUploadedPhotoFingerprints||{};renderAssetModeControls();renderAssetModeBanner();renderAssetPhotos();if(d.fields){Object.keys(d.fields).forEach(function(id){setAssetInput(id,d.fields[id]||"");});}renderAssetReplacementPanel();renderSavedAssets();updateDealUI();updateLocationUI();updateAssetSaveState();}
...
function renderAssetModeControls(){
  ...
  var add=el("asset-mode-add"),upd=el("asset-mode-update"),search=el("asset-search-card");
