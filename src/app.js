...
var FP_VERSION="210";
A.accountsMap={accounts:[],locations:[],missing:[],loading:false,map:null,markers:[],infoWindow:null,googleReady:null,pipeline:"All",stage:"",status:"Active",search:"",lastLoaded:0};
...
function accountsMapState(){if(!A.accountsMap)A.accountsMap={accounts:[],locations:[],missing:[],loading:false,map:null,markers:[],infoWindow:null,googleReady:null,pipeline:"All",stage:"",status:"Active",search:"",lastLoaded:0};return A.accountsMap;}
...
async function resolveAccountsMapLocation(account){var related=relatedDealsForAccount(account),meta=primaryDealMeta(related),shipping=accountsMapAddress(account,"Shipping"),billing=accountsMapAddress(account,"Billing"),direct=parseAccountsMapDirectCoords(account),base={account:account,relatedDeals:related,dealStage:meta.stage,pipeline:meta.pipeline,siteCoordsRaw:account.Latitude_Longitude||""};if(direct)return Object.assign(base,{lat:direct.lat,lng:direct.lng,source:"coords",displayAddress:shipping||"Coordinates on file",addressLabel:"Stored coordinates"});if(shipping){var shipGeo=await geocodeAccountsMapAddress(shipping);if(shipGeo)return Object.assign(base,{lat:shipGeo.lat,lng:shipGeo.lng,source:"shipping",displayAddress:shipping,addressLabel:"Shipping address",cached:shipGeo.cached});if(billing){var billAfterShip=await geocodeAccountsMapAddress(billing);if(billAfterShip)return Object.assign(base,{lat:billAfterShip.lat,lng:billAfterShip.lng,source:"billing",displayAddress:billing+" (billing address)",addressLabel:"Billing address",cached:billAfterShip.cached});}return{account:account,relatedDeals:related,reason:"Shipping geocode failed"+(billing?"; billing geocode failed":"")};}if(billing){var billGeo=await geocodeAccountsMapAddress(billing);if(billGeo)return Object.assign(base,{lat:billGeo.lat,lng:billGeo.lng,source:"billing",displayAddress:billing+" (billing address)",addressLabel:"Billing address",cached:billGeo.cached});return{account:account,relatedDeals:related,reason:"Billing geocode failed"};}return{account:account,relatedDeals:related,reason:"No stored coordinates, shipping address, or billing address"};}
...
function accountMatchesMapFilters(account,relatedDeals){var s=accountsMapState(),q=(s.search||"").toLowerCase();if(q&&(account.Account_Name||"").toLowerCase().indexOf(q)<0)return false;if(!accountStatusMatches(account,s.status||"Active"))return false;var deals=relatedDeals||[];if(s.stage&&deals.length&&!deals.some(function(d){return(d.Stage||"")===s.stage;}))return false;if(s.stage&&!deals.length)return false;if(s.pipeline&&s.pipeline!=="All"){if(!deals.length)return false;if(!deals.some(function(d){return normalizeAccountsMapPipeline(d.Pipeline)===s.pipeline;}))return false;}return true;}
function filteredAccountsMapLocations(){var s=accountsMapState();return s.locations.filter(function(loc){return accountMatchesMapFilters(loc.account,loc.relatedDeals);});}
function filteredAccountsMapMissing(){var s=accountsMapState();return s.missing.filter(function(item){return accountMatchesMapFilters(item.account,item.relatedDeals);});}
